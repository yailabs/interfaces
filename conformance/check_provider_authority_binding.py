...
SCHEMA_PATH = ROOT / "schemas/provider-authority-binding.v1.schema.json"
FIXTURE_DIR = ROOT / "fixtures/provider-authority-binding"
EXPECTED_FIXTURES = {
    "allowed-provider-call.json",
    "blocked-missing-case.json",
    "blocked-missing-entitlement.json",
    "blocked-license-expired.json",
    "provider-output-to-evidence.json",
}


def fail(message: str) -> None:
    print(f"provider-authority-binding: FAIL: {message}")
    raise SystemExit(1)
...
def main() -> None:
    ...
    for fixture_path in sorted(FIXTURE_DIR.glob("*.json")):
        ...
        require_keys(fixture, ["provider_authority"], "fixture", fixture_path)
        authority = require_object(
            fixture["provider_authority"], "provider_authority", fixture_path
        )
        require_keys(authority, authority_required, "provider_authority", fixture_path)

        require_enum(
            authority["requested_action"], actions, "requested_action", fixture_path
        )
        require_enum(authority["decision"], decisions, "decision", fixture_path)
        require_enum(
            authority["provider_output_posture"],
            output_postures,
        ...
        if decision != "requires_case" and not case_ref:
            fail(f"{display_path(fixture_path)} provider_authority.case_ref is required")
        ...
        for array_field in ("output_evidence_refs", "output_knowledge_refs"):
            if array_field in authority:
                require_string_array(authority[array_field], array_field, fixture_path)

    print("provider-authority-binding: ok")
